...
for i_wind in range(begins.size):
	...
	#print("Direct Time from L1 to H1 is %.3f seconds"%(dt_L1_H1))
	#print("Direct Time from L1 to V1 is %.3f seconds"%(dt_L1_V1))
	#print("Direct Time from H1 to V1 is %.3f seconds"%(dt_H1_V1))
	...
	np.savetxt("/lustre/gmorras/Triggered/Text/image_shiftH1_shiftV1_rL1H1_rL1V1_%.2f_%.2f.txt"%(start_data,end_data),image_shiftH1_shiftV1_rL1H1_rL1V1)
	...
	for t_det_sample in np.arange(start_data+t_det_overlap/2,end_data-t_det_overlap/2-t_det_step,t_det_step):
		...
		for i_image in i_sample:
			...
			#save figure
			plt.savefig("/lustre/gmorras/Triggered/Images/L1_H1_V1_%.3f_%.3f.png"%(t_image0_L1,t_imagef_L1), bbox_inches="tight")	
